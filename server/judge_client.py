...
class JudgeClient(object):
    ...
    def _judge_one(self, test_case_file_id):
        ...
        if run_result["result"] == _judger.RESULT_RUNTIME_ERROR:
            if os.path.exists(real_user_output_file):
                try:
                    with open(real_user_output_file, encoding="utf-8", errors="replace") as f:
                        logger.error(
                            f"[RTE] test_case={test_case_file_id}\n"
                            f"stderr:\n{f.read()}"
                        )
